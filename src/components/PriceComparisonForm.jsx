...
const PriceComparisonForm = () => {
  ...
  useEffect(() => {
    const fetchProcedures = async () => {
      try {
        ...
        const response = await axios.get("https://backend-service-4snlfkepaq-uc.a.run.app/api/procedures", {
          headers: {
  ...
  const handleOptionChange = async (event, newValue) => {
    if (newValue) {
      ...
      try {
        const token = sessionStorage.getItem("token");
        const response = await axios.get("https://backend-service-4snlfkepaq-uc.a.run.app/api/compare", {
  ...
  const saveComparison = async () => {
    ...
    try {
      const response = await axios.post(
        "https://backend-service-4snlfkepaq-uc.a.run.app/api/save-comparison",
        {
          comparison,
        },
        {
